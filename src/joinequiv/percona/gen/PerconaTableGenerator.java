...
import joinequiv.percona.PerconaSchema;
...
public class PerconaTableGenerator {
    ...
    private PerconaEngine engine;
    private int keysSpecified;
    private final List<String> columns = new ArrayList<>();
    private final PerconaSchema schema;
    private final PerconaGlobalState globalState;

    public PerconaTableGenerator(PerconaGlobalState globalState, String tableName) {
    ...
    public static SQLQueryAdapter generate(PerconaGlobalState globalState, String tableName) {
        return new PerconaTableGenerator(globalState, tableName).create();
    }
    ...
    private SQLQueryAdapter create() {
        ...
        if (Randomly.getBoolean() && !schema.getDatabaseTables().isEmpty()) {
            ...
            if (engine == PerconaEngine.CSV && (tableHasNullableColumn || setPrimaryKey)) {
                if (true) { // TODO
                    // results in an error
                    throw new IgnoreMeException();
                }
            } else if (engine == PerconaEngine.ARCHIVE && (tableHasNullableColumn || keysSpecified > 1)) {
    ...
    private void appendPartitionOptions() {
        if (engine != PerconaEngine.INNO_DB) {
            return;
        }
        if (Randomly.getBoolean()) {
            return;
        ...
        case HASH:
            ...
            // PerconaExpression expr =
            // PerconaRandomExpressionGenerator.generateRandomExpression(Collections.emptyList(),
            // null, r);
            // sb.append(PerconaVisitor.asString(expr));
    ...
    private void appendTableOptions() {
        ...
        for (TableOptions o : tableOptions) {
            ...
            case ENGINE:
                ...
                this.engine = PerconaEngine.get(fromOptions);
                sb.append("ENGINE = ");
...
//    private void appendColumnOption(PerconaDataType type) {
//        boolean isTextType = type == PerconaDataType.VARCHAR;
...
//                    sb.append(Randomly.fromOptions("DISK", "MEMORY"));
    ...
    private void appendColumnDefinition() {
        sb.append(" ");
        PerconaDataType randomType = PerconaDataType.getRandom(globalState);
        appendType(randomType);
        sb.append(" ");
        appendColumnOption(randomType);
    }

    private void appendType(PerconaDataType randomType) {
        switch (randomType) {
        case DECIMAL:
            sb.append("DECIMAL");
        ...
        if (randomType.isNumeric()) {
            if (Randomly.getBoolean() && randomType != PerconaDataType.INT && !PerconaBugs.bug99127) {
                sb.append(" UNSIGNED");
            }
    ...
    public static void optionallyAddPrecisionAndScale(StringBuilder sb) {
        if (Randomly.getBoolean() && !PerconaBugs.bug99183) {
            sb.append("(");
