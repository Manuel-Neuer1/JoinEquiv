...
import joinequiv.oceanbase.gen.OceanBaseAlterTable;
import joinequiv.oceanbase.gen.OceanBaseDeleteGenerator;
import joinequiv.oceanbase.gen.OceanBaseDropIndex;
import joinequiv.oceanbase.gen.OceanBaseInsertGenerator;
import joinequiv.oceanbase.gen.OceanBaseTableGenerator;
import joinequiv.oceanbase.gen.OceanBaseTruncateTableGenerator;
import joinequiv.oceanbase.gen.OceanBaseUpdateGenerator;
import joinequiv.oceanbase.gen.datadef.OceanBaseIndexGenerator;
...
//    }
    @Override
    public void generateDatabase(OceanBaseGlobalState globalState) throws Exception {
        // 1. 决定要创建的表的总数，确保至少为 2
        // Randomly.fromOptions(2, 3, 4) 会随机返回 2, 3, 或 4
        int numTablesToCreate = Randomly.fromOptions(2);

        // 2. 使用 for 循环，逻辑更清晰
        for (int i = 0; i < numTablesToCreate; i++) {
            String tableName = DBMSCommon.createTableName(i); // 使用循环变量 i 来命名
            SQLQueryAdapter createTable = OceanBaseTableGenerator.generate(globalState, tableName);
            globalState.executeStatement(createTable);
        }
        ...
        // (可选，如果你的测试需要表里有数据)
        // 在这里添加填充数据的逻辑，比如：
        // new InitialInsertExecutor(globalState).execute();

        // 后续的 StatementExecutor 逻辑保持不变
        StatementExecutor<OceanBaseGlobalState, Action> se = new StatementExecutor<>(globalState, Action.values(),
                OceanBaseProvider::mapActions, (q) -> {
            if (globalState.getSchema().getDatabaseTables().isEmpty()) {
                throw new IgnoreMeException();
