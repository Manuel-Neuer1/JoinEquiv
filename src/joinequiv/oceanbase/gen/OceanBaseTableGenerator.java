...
public class OceanBaseTableGenerator {
    ...
    private SQLQueryAdapter create() {
        ...
        if (Randomly.getBoolean() && !schema.getDatabaseTables().isEmpty()) {
            ...
            sb.append(" ");
            appendTableOptions();
            //appendPartitionOptions(); // TODO <-- 注释掉这一行！
...
//    }
    ...
    private void appendColumnDefinition() {
        ...
        // 1. 先把所有可选的选项放到一个列表里
        List<ColumnOptions> allOptions = new ArrayList<>(Arrays.asList(ColumnOptions.values()));
        // 2. 从列表中移除我们想要强制执行的选项
        allOptions.remove(ColumnOptions.NULL_OR_NOT_NULL);
        // 3. 从剩余的选项中随机选择一个子集
        List<ColumnOptions> columnOptions = Randomly.subset(allOptions);
        // 4. 把我们强制要执行的选项加回到列表的最前面（或任何位置）
        columnOptions.add(0, ColumnOptions.NULL_OR_NOT_NULL);
        // TODO 修改这段内容如上 强制每个列都有 not null 选项
