...
import joinequiv.sqlite3.ast.SQLite3Aggregate;
import joinequiv.sqlite3.ast.SQLite3Aggregate.SQLite3AggregateFunction;
import joinequiv.sqlite3.ast.SQLite3Case.CasePair;
import joinequiv.sqlite3.ast.SQLite3Case.SQLite3CaseWithBaseExpression;
import joinequiv.sqlite3.ast.SQLite3Case.SQLite3CaseWithoutBaseExpression;
import joinequiv.sqlite3.ast.SQLite3Constant;
import joinequiv.sqlite3.ast.SQLite3Constant.SQLite3TextConstant;
import joinequiv.sqlite3.ast.SQLite3Expression;
import joinequiv.sqlite3.ast.SQLite3Expression.BetweenOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.BinaryComparisonOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.BinaryComparisonOperation.BinaryComparisonOperator;
import joinequiv.sqlite3.ast.SQLite3Expression.CollateOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.Join;
import joinequiv.sqlite3.ast.SQLite3Expression.Join.JoinType;
import joinequiv.sqlite3.ast.SQLite3Expression.MatchOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3ColumnName;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3Distinct;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3OrderingTerm;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3OrderingTerm.Ordering;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3PostfixText;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3PostfixUnaryOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3PostfixUnaryOperation.PostfixUnaryOperator;
import joinequiv.sqlite3.ast.SQLite3Expression.SQLite3TableReference;
import joinequiv.sqlite3.ast.SQLite3Expression.Sqlite3BinaryOperation;
import joinequiv.sqlite3.ast.SQLite3Expression.Sqlite3BinaryOperation.BinaryOperator;
import joinequiv.sqlite3.ast.SQLite3Expression.TypeLiteral;
import joinequiv.sqlite3.ast.SQLite3Function;
import joinequiv.sqlite3.ast.SQLite3Function.ComputableFunction;
import joinequiv.sqlite3.ast.SQLite3RowValueExpression;
import joinequiv.sqlite3.ast.SQLite3Select;
import joinequiv.sqlite3.ast.SQLite3UnaryOperation;
import joinequiv.sqlite3.ast.SQLite3UnaryOperation.UnaryOperator;
import joinequiv.sqlite3.oracle.SQLite3RandomQuerySynthesizer;
import joinequiv.sqlite3.schema.SQLite3Schema.SQLite3Column;
import joinequiv.sqlite3.schema.SQLite3Schema.SQLite3Column.SQLite3CollateSequence;
import joinequiv.sqlite3.schema.SQLite3Schema.SQLite3RowValue;
import joinequiv.sqlite3.schema.SQLite3Schema.SQLite3Table;
...
public class SQLite3ExpressionGenerator implements ExpressionGenerator<SQLite3Expression>,
    ...
    public List<Join> getRandomJoinClauses(List<SQLite3Table> tables) {
        ...
        // 这是一个开关。它会检查全局配置，看用户是否在命令行中启用了 testJoins 选项。
        // 如果用户禁用了 JOIN 测试（比如为了专注于单表查询的测试），这个方法会直接返回空的 joinStatements 列表，从而阻止生成任何 JOIN 子句
        // 默认是 true
        if (!globalState.getDbmsSpecificOptions().testJoins) {
            return joinStatements;
        }
        // 获取所有可能的 JOIN 类型（比如 INNER, CROSS, OUTER, NATURAL, RIGHT, LEFT, FULL; 等）并存入一个 options 列表。这是后续随机选择的基础。
        List<JoinType> options = new ArrayList<>(Arrays.asList(JoinType.values()));
        if (Randomly.getBoolean() && tables.size() > 1) { // 50% 的概率并且要有两张表以上
            // 决定生成多少个 JOIN 子句
            // int nrJoinClauses = (int) Randomly.getNotCachedInteger(0, tables.size());
            int nrJoinClauses = (int) Randomly.getNotCachedInteger(1, 2);
            ...
            for (int i = 0; i < nrJoinClauses; i++) {
                // 生成 ON 条件
                // 调用 generateExpression() (return getRandomExpression(0)) 来创建一个随机的布尔表达式。
                // 这个表达式将作为 JOIN 的 ON 条件，例如 `ON t1.id = t2.user_id`。
                SQLite3Expression joinClause = generateExpression();
                // 【精妙之处】选择并移除表
                // 从传入的 tables 列表中随机选择一张表...
                SQLite3Table table = Randomly.fromList(tables);
                // ...然后立即将其从列表中移除。
                // 原因：这确保了同一张表不会在多个 JOIN 子句中被重复使用，避免生成类似 `JOIN t1 ... JOIN t1 ...` 这样的无效SQL。
                tables.remove(table);
                // 随机选择 JOIN 类型
                // 从可用的 JOIN 类型列表 `options` 中随机选择一种。
                JoinType selectedOption = Randomly.fromList(options);
                // 【精妙之处 】处理 NATURAL JOIN 的语法
                // 原因：`NATURAL JOIN` 语法上不允许有 `ON` 子句，它的连接条件是隐式的。
                // 措施：如果随机选中的是 `NATURAL JOIN`，就把之前生成的 ON 条件 (`joinClause`) 丢弃，并设置为空 (null)。
