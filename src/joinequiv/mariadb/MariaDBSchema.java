...
import joinequiv.mariadb.MariaDBSchema.MariaDBTable.MariaDBEngine;

public class MariaDBSchema extends AbstractSchema<MariaDBGlobalState, MariaDBTable> {
/* 这个类不生成任何修改数据库的 SQL 语句。它的工作是只读的。它通过查询 MariaDB 内置的 information_schema 数据库（这是一个存储了数据库元数据的地方），来精确地了解当前数据库的“样貌” */
/* 其他需要生成 SQL 的类（比如 MariaDBInsertGenerator 或 MariaDBUpdateGenerator）会向 MariaDBSchema 对象查询信息，以确保它们生成的 SQL 是有效的。例如，在生成一条 INSERT 语句之前，必须先问 MariaDBSchema：“数据库里有哪些表？我想插入的这张表有哪些列？这些列分别是什么数据类型？” */
    ...
    public static MariaDBSchema fromConnection(SQLConnection con, String databaseName) throws SQLException {
        ...
        for (int i = 0; i < NR_SCHEMA_READ_TRIES; i++) {
            try {
                List<MariaDBTable> databaseTables = new ArrayList<>(); // 数据库中所有表的列表
                try (Statement s = con.createStatement()) {
                    // 1. 查询 information_schema.TABLES 获取所有表名和存储引擎
