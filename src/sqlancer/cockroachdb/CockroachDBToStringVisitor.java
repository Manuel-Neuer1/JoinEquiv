...
//    }
    ...
    public void visit(CockroachDBSelect select) {
        ...
        if (select.isDistinct()) {
            sb.append("DISTINCT ");
        } else if (Randomly.getBoolean()) { // 这个逻辑也可以保留，虽然对于DQP来说总是DISTINCT
            sb.append("ALL ");
        }
        visit(select.getFetchColumns());
        sb.append(" FROM ");
        ...
        if (select.getJoinList().isEmpty()) {
            // 情况一：没有 JOIN。正常处理 FROM 列表（用于非JOIN查询）
            if (!select.getFromList().isEmpty()) {
                visit(select.getFromList());
            }
        } else {
            // 情况二：有 JOIN。直接访问 JOIN 列表，它会构建完整的 "t0 JOIN t1" 链。
            // 此时 fromList 应该被忽略，因为它已经被包含在 JOIN 的 leftTable 中了。
